...
@testset "energy" for Ni = [2], Nj = [2], D in [2], χ in [10]
    model = J1J2(Ni,Nj,1.0,0.5)
    A, key = init_ipeps(model; atype=CuArray, Ni=Ni, Nj=Nj, D=D, χ=χ)
    oc = optcont(D, χ)
    h = hamiltonian(model)
...
@testset "optimise_ipeps" for Ni = [2], Nj = [2], D in [2], χ in [10]
    ...
    optimise_ipeps(A, key; f_tol = 1e-6, opiter = 100, optimmethod = LBFGS(m = 20))
end